...
#ficha → onde apresento minhas informações pessoais.

#detalhes → onde coloco skills, formação e projetos.

Cada projeto foi estruturado com details e summary para permitir expansão de conteúdo.

As barras de progresso das skills funcionam com div internas que recebem largura em porcentagem (width: 80%, por exemplo).

No CSS, utilizei variáveis no :root para definir cores principais.
Criei a classe .escuro que altera essas variáveis, permitindo alternar entre tema claro e escuro.

No JavaScript:

Criei uma função para alternar o tema.
