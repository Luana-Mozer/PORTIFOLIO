...
function empresaValida(empresa) {
  const valor = String(empresa || '').trim().replace(/\s+/g, ' ');
  const partes = valor.split(' ');
  const caracteresPermitidos = /^[A-Za-zÀ-ÖØ-öø-ÿ0-9&.' -]+$/.test(valor);

  return caracteresPermitidos
    && partes.length >= 2
    && partes.every((parte) => parte.length >= 2 || ['&', 'e'].includes(parte.toLowerCase()))
    && valor.length <= 100
    && !textoPareceAleatorio(valor);
}
